...
\subsection{Uma versão diferente para o ciclo}

Robert Martin não descreve TDD como um ciclo, mas sim através de 3 regras \cite{bob-martin}:

\begin{enumerate}
	\item Não escreva nenhum código de produção até que você tenha um teste de unidade falhando;
	\item Não escreva mais do que um teste de unidade que seja suficiente para falhar (e não compilar significa falhar);
	\item Não escreva mais código de produção do que o suficiente para fazer o teste passar;
\end{enumerate}

Ao respeitar essas regras, o programador acaba caindo no ciclo definido anteriormente.
...
\section{Documentação executável}
...
O projeto AgileDox \footnote{http://http://agiledox.sourceforge.net/. Último acesso em 29/outubro/2010}, por exemplo, pega classes de teste e as 
...
É possível observer a crescente adoção e procura por TDD através do número de pesquisas publicadas pela academia (muitas delas citadas
no capítulo \ref{cap:trabalhos-relacionados}). Além disso, foi realizado o Primeiro Workshop Internacional de Test-Driven Development em 2010, 
na cidade de Paris, organizado pela Dra. Laurie Williams \footnote{http://agile.csc.ncsu.edu/tdd/. Último acesso em 29/outubro/2010}.

Em um questionário de 2010, organizado por Scott Wambler, chamado de \textit{How Agile Are You?} \cite{wambler-survey-agile}, mostra que 53\% dos times ágeis
adotaram TDD como uma maneira para validar o trabalho feito, conforme mostra a figura \ref{wambler-agile-2010}. Além disso, 
outro questionário de 2008, chamado de \textit{Test Driven Development (TDD) Survey} \cite{wambler-survey-tdd} mostra que 57\% dos desenvolvedores utilizam
TDD como técnica para capturar informações de design, conforme mostrado na figura \ref{wambler-tdd-2008}. Apesar de todos os possíveis vieses que
um questionário têm, os números mostram que uma grande parte dos desenvolvedor ágeis já utilizam a prática no seu dia-a-dia.
